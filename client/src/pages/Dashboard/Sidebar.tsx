...
import {BiSolidInstitution, BiTransfer} from 'react-icons/bi'
...
{(role === "user" ? menus : role === "admin" ? adminmenus : companymenus)?.map((menu, i)=>(
             ...
             <Link to={menu?.link} key={i} className={`${"mt-96"} group flex items-center text-sm text-left font-medium p-2 hover:text-black rounded-md`}>
              ...
              className={`whitespace-pre duration-500 ${!open && 'opacity-0 translate-x-28 overflown-hidden m-0'}`}>{menu?.name}</h2>
              <h2  className={`${
                  open && "hidden"
